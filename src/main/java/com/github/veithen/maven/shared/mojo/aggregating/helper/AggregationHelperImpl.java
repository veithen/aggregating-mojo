...
public class AggregationHelperImpl {
    private static final Map<MavenSession, Map<AggregationKey, Aggregator>> sessionMap =
            new WeakHashMap<>();

    public static List<Serializable> addResult(
        ...
        synchronized (aggregators) {
            aggregator =
                    aggregators.computeIfAbsent(
                            aggregationKey, k -> new Aggregator(mavenSession, k));
        }
